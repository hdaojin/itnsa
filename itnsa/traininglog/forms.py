...
class TrainingLogTypeForm(FlaskForm):
    name = StringField('类型名称', validators=[DataRequired(), Length(max=20, message='类型名称长度不能超过20个字符')])
    display_name = StringField('类型显示名称', validators=[DataRequired(), Length(max=20, message='类型显示名称长度不能超过20个字符')])
    description = StringField('类型描述', validators=[Length(max=100, message='类型描述长度不能超过100个字符')])
    submit = SubmitField('提交')
